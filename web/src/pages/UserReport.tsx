...
import { 
  Moon, Sun, LogOut, ArrowLeft, User, Phone, Mail, Users, Calendar, BarChart3, List
} from 'lucide-react';
...
export function UserReport() {
  ...
  const [visitMode, setVisitMode] = useState<'ocorrida' | 'marcada'>('ocorrida');
  
  // ESTADO QUE SEGURA O NOME NO CARD ATE CLICAR
  const [appliedVisitMode, setAppliedVisitMode] = useState<'ocorrida' | 'marcada'>('ocorrida');
        ...
        <main className="flex-1 p-8 overflow-y-auto bg-background/50">
          ...
          <div className="max-w-6xl mx-auto mb-8 bg-card border border-border p-4 rounded-xl flex flex-col md:flex-row gap-4 items-end shadow-sm">
             <div className="flex-1 space-y-2">
               <label className="text-xs font-bold uppercase text-muted-foreground ml-1">Data Inicial</label>
             ...
             <div className="flex-1 space-y-2">
               <label className="text-xs font-bold uppercase text-muted-foreground ml-1">Data Final</label>
               <div className="relative">
                 <Calendar className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground pointer-events-none" />
                 <Input 
                   type="text" 
             ...
             <div className="flex-1 space-y-2">
               <label className="text-xs font-bold uppercase text-muted-foreground ml-1">Modo de Visita</label>
               <Select value={visitMode} onValueChange={(val: any) => setVisitMode(val)}>
            ...
            <div className="max-w-6xl mx-auto space-y-8 pb-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                 ...
                 <div className="p-4 rounded-xl border border-border bg-card flex flex-col justify-center shadow-sm items-center text-center">
                   ...
                   <span className="text-3xl font-bold text-foreground">{data.metrics.uniqueLeadsContacted}</span>
                   <p className="text-[10px] uppercase font-bold text-muted-foreground mt-1 tracking-wider">Leads Contactados</p>
                 </div>

                 <div className="p-4 rounded-xl border border-border bg-card flex flex-col justify-center shadow-sm items-center text-center">
                   <List className="text-foreground mb-2" size={24} />
                   <span className="text-3xl font-bold text-foreground">{data.metrics.totalContacts}</span>
                   <p className="text-[10px] uppercase font-bold text-muted-foreground mt-1 tracking-wider">Total de Contatos</p>
                 </div>

                 <div className="p-4 rounded-xl border border-border bg-card flex flex-col justify-center shadow-sm items-center text-center">
                   <Phone className="text-green-500 mb-2" size={24} />
                   <span className="text-3xl font-bold text-foreground">{data.metrics.totalCalls}</span>
                   <p className="text-[10px] uppercase font-bold text-muted-foreground mt-1 tracking-wider">Nº Ligações</p>
                 </div>

                 <div className="p-4 rounded-xl border border-border bg-card flex flex-col justify-center shadow-sm items-center text-center">
                   <Mail className="text-orange-500 mb-2" size={24} />
                   <span className="text-3xl font-bold text-foreground">{data.metrics.totalEmails}</span>
                   <p className="text-[10px] uppercase font-bold text-muted-foreground mt-1 tracking-wider">Nº E-mails</p>
                 </div>

                 <div className="p-4 rounded-xl border border-border bg-card flex flex-col justify-center shadow-sm items-center text-center">
                   <Calendar className="text-purple-500 mb-2" size={24} />
                   <span className="text-3xl font-bold text-foreground">{data.metrics.totalVisits}</span>
                   <p className="text-[10px] uppercase font-bold text-muted-foreground mt-1 tracking-wider">Visitas ({appliedVisitMode})</p>
                 </div>
              </div>
                     ...
                     <table className="w-full text-sm text-left whitespace-nowrap">
                       <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b border-border sticky top-0 backdrop-blur-sm z-10">
                         <tr>
                           ...
                           <th className="px-6 py-4 font-medium">Etapa do Funil</th>
                           <th className="px-6 py-4 font-medium">Status / Etiqueta</th>
                           <th className="px-6 py-4 text-center font-medium">Vezes Contactado</th>
                           <th className="px-6 py-4 text-center font-medium">Mudanças de Funil</th>
                           <th className="px-6 py-4 text-center font-medium">Mudanças de Status</th>
                         </tr>
                       </thead>
                           ...
                           data.analyticalTable.map((row) => (
                             <tr key={row.leadId} className="hover:bg-muted/30 transition-colors">
                               ...
                               <td className="px-6 py-4">
                                  <Badge variant="outline">{reverseStageMap[row.funnel] || row.funnel}</Badge>
                               </td>
                               <td className="px-6 py-4">
                                  <Badge variant="secondary" className="uppercase text-[10px]">{row.status}</Badge>
                               </td>
